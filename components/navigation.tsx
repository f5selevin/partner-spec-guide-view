...
function Branch({ node, active }: { node: DocNode; active: string }) {
  ...
  return (
    <li>
      {node.sourcePath ? (
        <Link href={href} aria-current={selected ? "page" : undefined}>{node.title}</Link>
      ) : (
      ...
      {node.children.length > 0 && (
        <ul>
          {node.children.map((child) => (
            <Branch key={child.slug.join("/")} node={child} active={active} />
          ))}
        </ul>
      )}
    </li>
  ...
  return (
    <nav aria-label="Documentation">
      <ul className="nav-tree">
        {tree.map((node) => (
          <Branch key={node.slug.join("/")} node={node} active={active} />
        ))}
      </ul>
    </nav>
  );
